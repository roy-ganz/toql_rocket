...
use rocket::{State,requets::Form,http::Status};
use rocket_contrib::{databases::mysql, json::Json};
use serde::{Serialze, Deserialize};
use toql_rocket::{ToqlQuery, Counted, Result,mysql::load_many};
use toql_rocket::toql as toql;
...
fn main() {
    ...
    println!("----------------------------------------------------------------------------------------------------------------------");
    println!(
        "Create a user with `curl localhost:8000/user -X POST -d '{{\"username\":\"Peter\"}}'`"
    );
    println!(
        "Update a user with `curl localhost:8000/user/ID -X PUT -d '{{\"username\":\"Susan\"}}'`"
    );
